...
function revealAnswer(answer_id) {
	$.ajax({
		...
		success: function(data) {
			$("#reveal_btn").remove();
			$("#answer_container").html("<input type=\"hidden\" name=\"game_id\" value=\"1\" /><strong>Revealed:</strong> " + data);
		}
	});
}
